...
# total_perc <- country_cert %>%
#   group_by(certified) %>%
#   summarise(n=sum(n)) %>%
#   ungroup() %>%
#   mutate(perc=n/sum(n),
#          Country='Total %')
# country_perc <- rbind(country_cert, total_perc)
#
# Total % messes up ggplot2 plot ordering for some reason when trying to color.
...
completionCountry <- ggplot(subset(country_cert, certified=="Yes"), aes(x=reorder(Country, perc), y=perc)) +
  ...
  #geom_bar(data=subset(country_perc, (Country=='Total %'& certified=='Yes')), stat='identity', fill='black') +
  coord_flip() +
  scale_y_continuous(labels = percent) + 
  geom_text(aes(family="mono", fontface="bold", y=perc, label=paste(round(perc, 3)*100, "%", sep="")),
            color="white", size=3, hjust=1.5) + 
  theme_minimal() + ggtitle("Course Completion % by Country") +
  theme(text=element_text(family="Calibri")) + ylab("Percentage Who Earned Certification within Each Country") + xlab("Country")
...
total_age <- data.frame(Country = 'Total Average',
                        avg_age = mean(country_age$avg_age))

country_age <- rbind(country_age, total_age)

countryAge <- ggplot(country_age, aes(x=reorder(Country, avg_age), y=avg_age)) +
  theme_minimal() +
  geom_bar(stat="identity", fill="#A31F34") +
  geom_bar(data=subset(country_age, Country=='Total Average'), fill='black', stat='identity') + 
...
plotLevelEdu <- function(data=NULL, Title=NULL){
  ggplot(data, aes(x=Level_of_Edu, y=perc)) + 
  ...
  ggtitle(Title) + ylab("Percentage") + xlab("Level of Education") +
  theme_minimal() + 
  theme(text=element_text(family="Calibri"))  
}

e1 <- plotLevelEdu(data=edu, Title="Percentage with each Education Level for All Registered Students")
e2 <- plotLevelEdu(data=edu2, Title="Percentage with each Education Level for Active Users")
...
```{r, message=FALSE, warning=FALSE}
gradesEdu <- ggplot(subset(mooc_data, Active_User == "Active" & !is.na(Level_of_Edu)), 
                    aes(x=Level_of_Edu, y=grade, fill=Level_of_Edu)) +
  geom_boxplot() +
...
## Add a country for Total avg grade
total_grades <- data.frame(Country = 'Total Average',
                           avg_grade = mean(country_grade$avg_grade))

country_grade <- rbind(country_grade, total_grades)


gradesCountry <- ggplot(country_grade, aes(x=reorder(Country, avg_grade), y=avg_grade)) + 
  geom_bar(stat="identity", fill="#A31F34") +
  geom_bar(data=subset(country_grade, Country=='Total Average'), fill='black', stat='identity') + 
  coord_flip() +
  scale_y_continuous(labels = percent) + 
...
[4] http://minimaxir.com/2014/07/online-class-charts/
